        ...
        <div className="relative bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="sticky top-0 bg-white border-b border-slate-200 px-6 py-4 rounded-t-2xl">
            <div className="flex justify-between items-start">
              <div>
                <h2 className="text-xl font-bold text-navy-800 font-mono">
          ...
          <div className="px-6 py-4">
            {isLoading && (
              <div className="flex flex-col items-center justify-center py-12">
                <div className="w-12 h-12 border-4 border-navy-700 border-t-transparent rounded-full animate-spin mb-4" />
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-lg ${
                      analysis.risk_level === 'low' ? 'bg-emerald-50' :
                      analysis.risk_level === 'medium' ? 'bg-amber-50' : 'bg-red-50'
                    }`}>
                      <p className="text-sm text-slate-500">Risk Level</p>
                      <p className={`text-lg font-semibold capitalize ${
                        analysis.risk_level === 'low' ? 'text-emerald-700' :
                        analysis.risk_level === 'medium' ? 'text-amber-700' : 'text-red-700'
                      }`}>
                        {analysis.risk_level}
                      </p>
                    </div>
                    <div className={`p-4 rounded-lg ${
                      analysis.confidence === 'high' ? 'bg-emerald-50' :
                      analysis.confidence === 'medium' ? 'bg-amber-50' : 'bg-slate-50'
                    }`}>
                      <p className="text-sm text-slate-500">Confidence</p>
                      <p className={`text-lg font-semibold capitalize ${
                        analysis.confidence === 'high' ? 'text-emerald-700' :
                        analysis.confidence === 'medium' ? 'text-amber-700' : 'text-slate-700'
                      }`}>
                        {analysis.confidence}
                      </p>
                    </div>
